...
func (this *Sitemap) CreateSitemapIndex(si []SitemapIndex, file string) error {
	var xmls = []string{fmt.Sprintf(`<?xml version="%v" encoding="%v"?><sitemapindex>`, this.Version, this.Encoding)}
	for _, v := range si {
		xml := ""
		if len(v.Lastmod) > 0 {
	...
	return ioutil.WriteFile(file, []byte(strings.Join(xmls, "")), os.ModePerm)
}
...
func (this *Sitemap) CreateSitemapContent(su []SitemapUrl, file string) error {
	var xmls = []string{fmt.Sprintf(`<?xml version="%v" encoding="%v"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">`, this.Version, this.Encoding)}
	for _, v := range su {
